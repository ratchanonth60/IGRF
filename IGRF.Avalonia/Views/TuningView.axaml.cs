...
namespace IGRF.Avalonia.Views
{
    public partial class TuningView : UserControl
    {
        ...
        public TuningView()
        {
            ...
            _streamerX1 = ScottPlotThemeHelper.SetupDataStreamer(PlotX, "Sensor X", AppConstants.Colors.CyanBlue);
            _streamerY1 = ScottPlotThemeHelper.SetupDataStreamer(PlotY, "Sensor Y", AppConstants.Colors.SkyBlue);
            _streamerZ1 = ScottPlotThemeHelper.SetupDataStreamer(PlotZ, "Sensor Z", AppConstants.Colors.SlateBlue);
        ...
        {
            // Support both MainViewModel (legacy) and TuningViewModel (new MVVM)
            if (DataContext is ViewModels.TuningViewModel tuningVm)
            {
                tuningVm.DataUpdated += OnDataUpdated;
                tuningVm.DualDataUpdated += OnDualDataUpdated;
            }
            else if (DataContext is ViewModels.MainViewModel mainVm)
            {
                mainVm.DataUpdated += OnDataUpdated;
                mainVm.DualDataUpdated += OnDualDataUpdated;
            }
        }
